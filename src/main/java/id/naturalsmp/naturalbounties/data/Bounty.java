package me.naturalsmp.NaturalBounties.data;
...
import me.naturalsmp.NaturalBounties.features.BountyExpire;
...
public class Bounty extends Inconsistent implements Comparable<Bounty> {
    ...
    public void setDisplayName(@NotNull String name) {
        try {
            ...
            Logger logger = NaturalBounties.getInstance().getLogger();
            NaturalBounties.getInstance().getLogger().log(Level.WARNING, "Bounty name cannot be null", e);
    ...
    public UUID getServerID() {
        if (serverID == null) {
            NaturalBounties.debugMessage("No server ID set for bounty!", true);
