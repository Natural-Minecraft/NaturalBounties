package me.naturalsmp.NaturalBounties.features.challenges;
...
import me.naturalsmp.NaturalBounties.ui.gui.bedrock.GUIComponent;
...
import me.naturalsmp.NaturalBounties.features.settings.auto_bounties.MurderBounties;
import me.naturalsmp.NaturalBounties.features.settings.auto_bounties.RandomBounties;
...
public class ChallengeManager implements Listener {
    ...
    public static void reloadOptions() {
        File challengeFile = new File(NaturalBounties.getInstance().getDataFolder() + File.separator + "challenges/challenges.yml");
        if (!challengeFile.exists()) {
            NaturalBounties.getInstance().saveResource("challenges/challenges.yml", false);
        }
            ...
            try {
                ...
                if (key.equals("custom-challenges")) {
                    if (ConfigOptions.getIntegrations().isPapiEnabled()) {
                        ...
                        NaturalBounties.debugMessage("Custom challenges are set in the config, but PlaceholderAPI is not enabled!", true);
                    }
                } else {
                    // built-in challenges
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Invalid challenge type: " + key);
            }
        }
    }
    ...
    public static void readChallengeData() throws IOException {
        ...
        File challengeFolder = new File(NaturalBounties.getInstance().getDataFolder() + File.separator + "challenges");
        ...
        for (String key : configuration.getKeys(false)) {
            ...
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Could not convert string to uuid in challenge-data.yml: " + key);
                continue;
            }
            ...
            for (String challengeIndexString : Objects.requireNonNull(configuration.getConfigurationSection(key)).getKeys(false)) {
                ...
                try {
                    ...
                    Bukkit.getLogger().warning("[NaturalBounties] Could not get challenge index for UUID: " + key);
                    continue;
                }
                // check for a valid challenge
                if (challengeIndex >= allChallenges.size()) {
                    Bukkit.getLogger().warning(() -> "[NaturalBounties] Saved challenge no longer exists! (" + challengeIndex + ")");
                    continue;
                }
                ...
                if (configuration.isSet(key + "." + challengeIndexString + ".variation")) {
                    ...
                    if (variationIndex >= challenge.getVariations().size()) {
                        ...
                        Bukkit.getLogger().warning(() -> "[NaturalBounties] Saved challenge variation no longer exists! (" + challengeIndex + ":" + finalVariationIndex + ")");
                        variationIndex = 0;
                    }
    ...
    public static void saveChallengeData() throws IOException {
        ...
        if (challengeFolder.mkdir()) {
            Bukkit.getLogger().info("[NaturalBounties] Created new challenges folder.");
        }
        File challengeDataFile = new File(challengeFolder + File.separator + "challenge-data.yml");
        if (challengeDataFile.createNewFile()) {
            Bukkit.getLogger().info("[NaturalBounties] Created new challenge data file.");
        }
        ...
        if (globalChallenges) {
            ...
            for (int i = 0; i < challenges.size(); i++) {
                Challenge challenge = challenges.get(i).challenge();
                // get challenge index
                ...
                if (challengeIndex == -1) {
                    ...
                    NaturalBounties.debugMessage("Error getting index of challenge: " + challenge.getChallengeType(), false);
                    continue;
                }
                // get variation index - already stored as indices in the list
                int variationIndex = challenges.get(i).variationIndex();
                if (challenge.getVariations().size() <= variationIndex) {
                    ...
                    NaturalBounties.debugMessage("Error getting variation: " + challenge.getChallengeType() + ":" + i, true);
                    variationIndex = 0;
                }
            ...
            for (Map.Entry<UUID, LinkedList<ActiveChallenge>> entry : activeChallenges.entrySet()) {
                ...
                for (int i = 0; i < activeChallengeList.size(); i++) {
                    Challenge challenge = activeChallengeList.get(i).challenge();
                    int challengeIndex = getChallengeIndex(challenge);
                    if (challengeIndex == -1) {
                        // debug only because this may happen when a challenge is removed while the server is running
                        NaturalBounties.debugMessage("Error getting index of challenge: " + challenge.getChallengeType(), true);
                        continue;
                    }

                    // get variation index - already stored as indices in the list
                    int variationIndex = activeChallengeList.get(i).variationIndex();
                    if (challenge.getVariations().size() <= variationIndex) {
                        // debug only because this may happen when a variation is removed while the server is running
                        NaturalBounties.debugMessage("Error getting variation: " + challenge.getChallengeType() + ":" + i, true);
                        variationIndex = 0;
    ...
    private static void generateProgress(Player player) {
        ...
        for (ActiveChallenge createdChallenge : createdChallenges) {
            Challenge challenge = createdChallenge.challenge();
            ...
            if (challenge.getChallengeType() == ChallengeType.BOUNTY_MULTIPLE) {
                goal = progressValue * challenge.getVariations().get(createdChallenge.variationIndex());
            } else {
                goal = progressValue + challenge.getVariations().get(createdChallenge.variationIndex());
            }
    ...
    public static void updateChallengeProgress(UUID uuid, ChallengeType challengeType, double progressChange) {
        ...
        for (int i = 0; i < playerChallenges.size(); i++) {
            ...
            if (challenge.challenge().getChallengeType().equals(challengeType)) {
                ChallengeData challengeData = getChallengeData(uuid, i);
                if (challengeType == ChallengeType.CLOSE_BOUNTY && progressChange < challenge.challenge().getVariations().get(challenge.variationIndex())) {
    ...
    private static void updateCustomChallenges() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            for (int i = 0; i < activeChallengeList.size(); i++) {
                ...
                if (activeChallenge.challenge().getChallengeType() == ChallengeType.CUSTOM && !challengeData.isNotified()) {
                    // update progress
                    double currentValue = activeChallenge.challenge().getCustomProgress(player);
                    challengeData.addProgress(currentValue -  challengeData.getProgress());

                    // check for completion
                    if (activeChallenge.challenge().isCustomRequirementCompleted(player, challengeData.getGoal())) {
    ...
    private static boolean isChallengeEnabled(Challenge challenge) {
        switch (challenge.getChallengeType()) {
            case PURCHASE_IMMUNITY -> {
                return ImmunityManager.getImmunityType() == ImmunityManager.ImmunityType.TIME || ImmunityManager.getImmunityType() == ImmunityManager.ImmunityType.SCALING;
            }
            case HOLD_POSTER -> {
                return BountyMap.isCraftPoster();
            }
            case WHITELISTED_BOUNTY_SET -> {
                return Whitelist.isVariableWhitelist();
            }
            case AUTO_BOUNTY -> {
                return MurderBounties.isEnabled() || RandomBounties.isEnabled() || TimedBounties.isEnabled();
            }
            case BUY_OWN -> {
                return ConfigOptions.getMoney().isBuyOwn();
            }
            default -> {
                return true;
            }
    ...
    private static double getVariation(UUID uuid, int challengeIndex) {
        if (globalChallenges) {
            ...
            return activeChallenges.get(DataManager.GLOBAL_SERVER_ID).get(challengeIndex).challenge().getVariations().get(activeChallenges.get(DataManager.GLOBAL_SERVER_ID).get(challengeIndex).variationIndex());
        } else {
            if (!activeChallenges.containsKey(uuid))
                return 0;
            // return the active challenge variation for uuid argument
            return activeChallenges.get(uuid).get(challengeIndex).challenge().getVariations().get(activeChallenges.get(uuid).get(challengeIndex).variationIndex());
        }
    }
    ...
    public static String getChallengeTitle(OfflinePlayer player, int challengeIndex) {
        ...
        int variationIndex = activeChallenge.variationIndex();
    ...
    private static double getStartAmount(UUID uuid, double goal, int challengeIndex) {
        ...
        double variation = activeChallenge.challenge().getVariations().get(activeChallenge.variationIndex());
        if (activeChallenge.challenge().getChallengeType() == ChallengeType.BOUNTY_MULTIPLE) {
    ...
    public static ItemStack[] getDisplayItems(Player player) {
        ...
        for (int i = 0; i < concurrentChallenges; i++) {
            ActiveChallenge activeChallenge = activeChallengeList.get(i);
            ChallengeData challengeData = challengeDataList.get(i);
            items[i] = activeChallenge.challenge().getItem(player, getStartAmount(player.getUniqueId(), challengeData.getGoal(), i), challengeData.getProgress(), challengeData.getGoal(), activeChallenge.variationIndex(), !challengeData.isRewarded() && challengeData.isNotified());
        }
        return items;
    }
    ...
    public static List<String> getDisplayText(Player player, int challengeIndex) {
        ChallengeData challengeData = getChallengeData(player.getUniqueId(), challengeIndex);
        ActiveChallenge activeChallenge = getActiveChallenges(player.getUniqueId()).get(challengeIndex);
        return activeChallenge.challenge().getDisplayText(player, getStartAmount(player.getUniqueId(), challengeData.getGoal(), challengeIndex), challengeData.getProgress(), challengeData.getGoal(), activeChallenge.variationIndex(), !challengeData.isRewarded() && challengeData.isNotified());
    }
    ...
    public static GUIComponent[] getDisplayComponents(Player player) {
        ...
        for (int i = 0; i < concurrentChallenges; i++) {
            ActiveChallenge activeChallenge = activeChallengeList.get(i);
            ChallengeData challengeData = challengeDataList.get(i);
            items[i] = activeChallenge.challenge().getComponent(player, getStartAmount(player.getUniqueId(), challengeData.getGoal(), i), challengeData.getProgress(), challengeData.getGoal(), activeChallenge.variationIndex(), !challengeData.isRewarded() && challengeData.isNotified());
            items[i].addCommands(List.of("[p] " + ConfigOptions.getPluginBountyCommands().get(0) + " challenges claim " + (i+1)));
        }
        return items;
    }
    ...
    public static boolean tryClaim(Player player, int challengeIndex) {
        ...
        if (challengeData.getProgress() >= challengeData.getGoal() && !challengeData.isRewarded()) {
            ActiveChallenge activeChallenge = getActiveChallenges(player.getUniqueId()).get(challengeIndex);
            activeChallenge.challenge().executeCommands(player, getStartAmount(player.getUniqueId(), challengeData.getGoal(), challengeIndex), challengeData.getProgress(), challengeData.getGoal(), activeChallenge.variationIndex());
