...
import me.naturalsmp.NaturalBounties.features.settings.display.Display;
import me.naturalsmp.NaturalBounties.features.settings.immunity.Immunity;
...
import me.naturalsmp.NaturalBounties.features.settings.money.Money;
...
import me.naturalsmp.NaturalBounties.ui.gui.GUIClicks;
...
import me.naturalsmp.NaturalBounties.features.settings.auto_bounties.AutoBounties;
import me.naturalsmp.NaturalBounties.features.challenges.ChallengeManager;
import me.naturalsmp.NaturalBounties.features.settings.databases.Databases;
...
public class ConfigOptions {
    ...
    public static void reloadOptions(Plugin plugin) throws IOException {
        ...
        if (NaturalBounties.getInstance().getResource("config.yml") != null) {
            plugin.getConfig().setDefaults(YamlConfiguration.loadConfiguration(new InputStreamReader(Objects.requireNonNull(NaturalBounties.getInstance().getResource("config.yml")))));
        ...
        if (pluginBountyCommands.isEmpty())
            pluginBountyCommands.add("NaturalBounties");
        if (firstStart)
        ...
        if (guiConfig.getKeys(true).size() <= 2) {
            ...
            if (NaturalBounties.getInstance().getResource("gui.yml") != null) {
                guiConfig = YamlConfiguration.loadConfiguration(new InputStreamReader(Objects.requireNonNull(NaturalBounties.getInstance().getResource("gui.yml"))));
    ...
    private static void registerAliases(List<String> aliases) {
        ...
        aliases.removeIf(s -> s.equalsIgnoreCase("NaturalBounties")); // this is already the command name
        PluginCommand command = NaturalBounties.getInstance().getCommand("NaturalBounties");
        if (command == null) {
        ...
        try {
            ...
            for (String alias : aliases) {
                commandMap.register(alias, "NaturalBounties", command);
            }
    ...
    public static void saveConfigurationSection(String resourceName, YamlConfiguration configuration, String section) {
        if (NaturalBounties.getInstance().getResource(resourceName) != null) {
            YamlConfiguration resourceConfig = YamlConfiguration.loadConfiguration(new InputStreamReader(Objects.requireNonNull(NaturalBounties.getInstance().getResource(resourceName))));
    ...
    public static void setUpdateNotification(String updateNotification) {
        ...
        NaturalBounties.getInstance().reloadConfig();
        if (NaturalBounties.getInstance().getConfig().getKeys(true).size() <= 2) {
            plugin.getLogger().severe("Loaded an empty configuration for the config.yml file. Fix the YAML formatting errors, or the plugin may not work!\nFor more information on YAML formatting, see here: https://spacelift.io/blog/yaml");
            return;
        }
        NaturalBounties.getInstance().getConfig().set("update-notification", updateNotification);
        NaturalBounties.getInstance().saveConfig();
    }
    ...
    public static void runGUIPluginCommand(CommandSender sender, String command) {
        if (!Bukkit.isPrimaryThread()) {
            NaturalBounties.getServerImplementation().global().run(() -> runGUIPluginCommand(sender, command));
        } else {
            if (usePlcmdInGui) {
                Bukkit.dispatchCommand(sender, pluginBountyCommands.get(0) + " " + command);
            } else {
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "NaturalBountiesadmin " + sender.getName() + " " + command);
            }
        }
    }
