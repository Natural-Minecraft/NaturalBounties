package me.naturalsmp.NaturalBounties.features.webhook;
...
import me.naturalsmp.NaturalBounties.utils.tasks.WebhookBuilder;
...
public class WebhookOptions implements Listener {
    ...
    public static File getFile() {
        return new File(NaturalBounties.getInstance().getDataFolder() + File.separator + "webhook.yml");
    }

    public static void reloadOptions() throws IOException {
        if (!getFile().exists())
            NaturalBounties.getInstance().saveResource("webhook.yml", false);
        YamlConfiguration configuration = YamlConfiguration.loadConfiguration(getFile());

        boolean saveChanges = true;
        if (configuration.getKeys(true).size() <= 2) {
            saveChanges = false;
            NaturalBounties.getInstance().getLogger().severe("Loaded an empty configuration for the webhook.yml file. Fix the YAML formatting errors, or the plugin may not work!\nFor more information on YAML formatting, see here: https://spacelift.io/blog/yaml");
        }
    ...
    private void buildWebhook(Webhook webhook, UUID receiver, UUID player, double amount, double total){
        if (webhook.isEnabled()) {
            ...
            webhookBuilder.setTaskImplementation(NaturalBounties.getServerImplementation().async().runAtFixedRate(webhookBuilder, 1, 4));
        }
