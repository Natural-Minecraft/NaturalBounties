package me.naturalsmp.NaturalBounties.features.settings.databases.proxy;
...
import me.naturalsmp.NaturalBounties.ui.PlayerSkin;
import me.naturalsmp.NaturalBounties.ui.SkinManager;
import me.naturalsmp.NaturalBounties.utils.BountyChange;
...
import me.naturalsmp.NaturalBounties.utils.tasks.SkinRequest;
...
public class ProxyMessaging implements PluginMessageListener, Listener {
    ...
    protected static final String CHANNEL = "NaturalBounties:main";
    ...
    private void receiveMessage(ByteArrayDataInput in) throws IOException {
        ...
        NaturalBounties.debugMessage("Received a message from proxy: " + subChannel, false);
        ...
        switch (subChannel) {
            ...
            case "PlayerSkin" -> {
                try {
                    ...
                    Bukkit.getLogger().warning("[NaturalBounties] Invalid skin URI");
    ...
    private synchronized void receiveConnection(DataInputStream msgIn) throws IOException {
        ...
        for (short i = 0; i < savedStats; i++) {
            ...
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Invalid uuid in received proxy message.");
            }
            if (uuid != null)
        ...
        try {
            ...
            // proxy isn't updated
            Bukkit.getLogger().severe("[NaturalBounties] Player data has not been transmitted from the proxy. Is the proxy NaturalBounties updated?");
        }
        ...
        try {
            ...
            // proxy isn't updated
            Bukkit.getLogger().severe("[NaturalBounties] Message count has not been transmitted from the proxy. Is the proxy NaturalBounties updated?");
            currentMessage = 1;
            remainingMessages = 0;
        }
        if (currentMessage == 1) {
    ...
    public static void sendMessage(String identifier, byte[] data) {
        if (ProxyDatabase.isEnabled() && NaturalBounties.getInstance().isEnabled()) {
    ...
    public static void sendMessage(String identifier, byte[] data, Player player) {
        player.sendPluginMessage(NaturalBounties.getInstance(), identifier, data);
    ...
    public static void sendBountyUpdate(List<BountyChange> bountyChanges) {
        // send proxy message
        NaturalBounties.debugMessage("Sending Bounty Update of " + bountyChanges.size() + " changes.", false);
    ...
    public static void sendStatUpdate(Map<UUID, PlayerStat> statChanges) {
        // send proxy message
        NaturalBounties.debugMessage("Sending Stat Update of " + statChanges.size() + " changes.", false);
        List<String[]> encoded = new LinkedList<>();
    ...
    public static void sendPlayerDataUpdate(List<PlayerData> playerDataList) {
        NaturalBounties.debugMessage("Sending PlayerData Update of " + playerDataList.size() + " changes.", false);
        List<String[]> encoded = new LinkedList<>();
    ...
    public static void requestPlayerList(){
        ...
        try {
            ...
            NaturalBounties.debugMessage("Error preparing a player list request", true);
        ...
        NaturalBounties.debugMessage("Sent player list request.", false);
    }
        ...
        if (ProxyDatabase.areSkinRequestsEnabled()) {
            NaturalBounties.getServerImplementation().global().run(() -> {
                if (Bukkit.getOnlinePlayers().isEmpty()) {
                    ...
                    skinRequest.setTaskImplementation(NaturalBounties.getServerImplementation().global().runAtFixedRate(skinRequest,5, 5));
                }
            });
        }
    }
    ...
    public static void sendSkinRequest(UUID uuid) {
        ...
        out.write(msgBytes.toByteArray()); // This is the message.
        sendMessage(CHANNEL, out.toByteArray());
        NaturalBounties.debugMessage("Sent player skin request.", false);
    }
    ...
    public static void onCompleteProxyMessage(long id) {
        ...
        NaturalBounties.debugMessage("Complete proxy message not found. id: " + id, true);
    }
    ...
    public static void onFailProxyMessage(long id) {
        ...
        NaturalBounties.debugMessage("Failed proxy message not found. id: " + id, true);
    }
