...
public class SkinsRestorerClass {
    ...
    private boolean connect(){
        try {
            ...
            if (!Bukkit.getOnlinePlayers().isEmpty()) {
                ...
                NaturalBounties.getServerImplementation().global().runDelayed(() -> firstConnect = false, 5 * 20L);
            }
            if (!firstConnect && lastHookError < System.currentTimeMillis() && !(ProxyMessaging.hasConnectedBefore() && ProxyDatabase.areSkinRequestsEnabled())) {
                    Bukkit.getLogger().warning("[NaturalBounties] Failed at hooking into SkinsRestorer, will try again on next call.");
    ...
    private TaskImplementation<Void> createDelayedCheckTask(long tickDelay) {
        return NaturalBounties.getServerImplementation().global().runDelayed(() -> {
            ...
            while (mapIterator.hasNext()) {
                ...
                if (entry.getValue() - System.currentTimeMillis() < MAX_DELAY - MIN_DELAY) {
                    ...
                    if (!SkinManager.isSkinLoaded(entry.getKey())) {
                        NaturalBounties.debugMessage("Proxy skin request timed out for player: " + entry.getKey().toString(), true);
    ...
    public void saveSkin(UUID uuid) {
        ...
        try {
            ...
            if (skinProperty.isEmpty()) {
                NaturalBounties.debugMessage("[NaturalBountiesDebug] Skin property not present from SkinsRestorer for " + name + ".", true);
                requestSkinManually(uuid);
                return;
            }
            ...
            NaturalBounties.debugMessage("[NaturalBountiesDebug] Error getting skin from SkinsRestorer.", true);
