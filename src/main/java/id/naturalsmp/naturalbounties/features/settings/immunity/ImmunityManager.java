package me.naturalsmp.NaturalBounties.features.settings.immunity;
...
import me.naturalsmp.NaturalBounties.utils.tasks.LoadPlaytimeTask;
...
public class ImmunityManager {
    public enum ImmunityType {
        ...
        DISABLE,
        /**
         * Permanent immunity - bought or with the NaturalBounties.immune permission
         */
        PERMANENT,
        /**
    ...
    /**
     * Whether the NaturalBounties.immune permission and other permission nodes gives players immunity
     */
    ...
    public static void loadConfiguration(ConfigurationSection configuration) {
        ...
        try {
            ...
            Bukkit.getLogger().warning("[NaturalBounties] Immunity type is not set to a proper value!");
        }
        ...
        if (oldNewPlayerImmunity != newPlayerImmunity) {
            ...
            if (newPlayerImmunity > 0) {
                ...
                loadPlaytimeTask.setTaskImplementation(NaturalBounties.getServerImplementation().global().runAtFixedRate(loadPlaytimeTask, 1, 1));
            } else {
                // disabled
    ...
    public static void checkPermissionImmunity(Player player) {
        ...
        playerData.setGeneralImmunity(player.hasPermission("NaturalBounties.immune"));
        playerData.setTimedImmunity(player.hasPermission("NaturalBounties.immunity.timed"));
        playerData.setRandomImmunity(player.hasPermission("NaturalBounties.immunity.random"));
        playerData.setMurderImmunity(player.hasPermission("NaturalBounties.immunity.murder"));
        if (playerData.isNewPlayer())
            NaturalBounties.getServerImplementation().global().run(() -> playerData.setNewPlayer(((double) player.getStatistic(Statistic.PLAY_ONE_MINUTE) / 1200) < newPlayerImmunity));
