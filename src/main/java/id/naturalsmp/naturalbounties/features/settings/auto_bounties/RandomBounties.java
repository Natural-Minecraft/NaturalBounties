package me.naturalsmp.NaturalBounties.features.settings.auto_bounties;

import com.cjcrafter.foliascheduler.TaskImplementation;
import me.naturalsmp.NaturalBounties.NaturalBounties;
import me.naturalsmp.NaturalBounties.utils.BanChecker;
...
import static me.naturalsmp.NaturalBounties.utils.BountyManager.addBounty;
...
public class RandomBounties {
    ...
    public static void update() {
        ...
        if (randomBountyMinTime != 0 && nextRandomBounty != 0 && System.currentTimeMillis() > nextRandomBounty) {
            if (!randomBountyOfflineSet && NaturalBounties.getNetworkPlayers().isEmpty()) {
                setNextRandomBounty();
                return;
            }
            UUID uuid = randomBountyOfflineSet ? (UUID) LoggedPlayers.getLoggedPlayers().keySet().toArray()[random.nextInt(LoggedPlayers.getLoggedPlayers().size())] : (UUID) NaturalBounties.getNetworkPlayers().keySet().toArray()[random.nextInt(NaturalBounties.getNetworkPlayers().size())];
            ...
            try {
                ...
                NaturalBounties.getServerImplementation().async().runNow(task -> {
                    if (!BanChecker.isPlayerBanned(player)) {
                        ...
                        NaturalBounties.getServerImplementation().global().run((Consumer<TaskImplementation<Void>>) task1 ->
                ...
                Bukkit.getLogger().info("[NaturalBounties] Invalid UUID of picked player for random bounty: " + uuid);
            }
        }
    }
    ...
    private static boolean hasImmunity(OfflinePlayer player) {
        if (!ImmunityManager.isPermissionImmunity())
            return false;
        if (player.isOnline())
            return Objects.requireNonNull(player.getPlayer()).hasPermission("NaturalBounties.immunity.random");
