...
import static me.naturalsmp.NaturalBounties.utils.BountyManager.getAllBounties;
...
public class BigBounty {
    ...
    public static void loadConfiguration(ConfigurationSection configuration) {
        ...
        try {
            ...
            Bukkit.getLogger().warning("[NaturalBounties] Invalid type for big bounty trigger \"" + configuration.getString("trigger") + "\". (ONCE, AMOUNT, SET)");
        ...
        for (Iterator<UUID> iterator = particlePlayers.iterator(); iterator.hasNext(); ) {
            ...
            if (player != null) {
                for (Player viewer : Bukkit.getOnlinePlayers()) {
                    if ((NaturalBounties.getServerVersion() >= 17 && viewer.canSee(Objects.requireNonNull(player.getPlayer())))
                            ...
                            && !NaturalBounties.isVanished(player.getPlayer()))
