...
public class LocalTime {
    ...
    public static void readAuthentication() throws IOException {
        ...
        File secretFile = new File(NaturalBounties.getInstance().getDataFolder() + File.separator + SECRET_FILE);
        if (!secretFile.exists()) {
            NaturalBounties.getInstance().saveResource(SECRET_FILE, false);
            NaturalBounties.debugMessage("Created Secret File!", false);
        }
        YamlConfiguration configuration;
        if (secretFile.exists()) {
            configuration = YamlConfiguration.loadConfiguration(secretFile);
        } else if (NaturalBounties.getInstance().getResource(SECRET_FILE) != null) {
            configuration = YamlConfiguration.loadConfiguration(new InputStreamReader(Objects.requireNonNull(NaturalBounties.getInstance().getResource(SECRET_FILE))));
        } else {
    ...
    private static String formatTime(long time, Player player) {
        ...
        if (license == null) {
            try {
                ...
                NaturalBounties.debugMessage(e.getMessage(), false);
            }
        }
        ...
        NaturalBounties.getServerImplementation().async().runNow(() -> registerTimeZone(player));
...
class ResponseHandler implements HttpClientResponseHandler<TimeZone> {
    ...
    public TimeZone handleResponse(ClassicHttpResponse classicHttpResponse) throws HttpException, IOException {
        ...
        if (entity != null) {
            ...
            NaturalBounties.debugMessage("Receieved Timezone Response!", false);
            JsonObject input;
            try {
                input = new JsonParser().parse(result).getAsJsonObject();
            } catch (JsonSyntaxException e) {
                NaturalBounties.debugMessage("Bad Syntax.", false);
                throw new IOException("Bad Syntax.");
            }
            if (input.has("location")) {
                NaturalBounties.debugMessage("Valid location!", false);
                JsonObject location = input.getAsJsonObject("location");
                return TimeZone.getTimeZone(location.get("time_zone").getAsString());
            } else {
                NaturalBounties.debugMessage(input.toString(), false);
