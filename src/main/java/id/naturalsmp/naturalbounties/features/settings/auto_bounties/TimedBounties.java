...
import static me.naturalsmp.NaturalBounties.utils.BanChecker.isPlayerBanned;
import static me.naturalsmp.NaturalBounties.NaturalBounties.isVanished;
...
public class TimedBounties {
        ...
        for (Map.Entry<UUID, Long> entry : nextBountiesCopy.entrySet()) {
            ...
            if (System.currentTimeMillis() > entry.getValue()) {
                if (player.isOnline() && isVanished(Objects.requireNonNull(player.getPlayer())))
                    continue;
                // set bounty
                TaskImplementation<Boolean> checkBounty = NaturalBounties.getServerImplementation().async().runNow(task -> { return isPlayerBanned(player); });
                checkBounty.asFuture().thenRun(() -> NaturalBounties.getServerImplementation().global().run(nextTask -> {
    ...
    private static boolean hasImmunity(OfflinePlayer player) {
        if (!ImmunityManager.isPermissionImmunity())
            return false;
        if (player.isOnline())
            return Objects.requireNonNull(player.getPlayer()).hasPermission("NaturalBounties.immunity.timed");
