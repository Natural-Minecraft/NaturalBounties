...
public class EntityTag extends TagProvider {
    ...
    public void updateVisibility() {
        if (NaturalBounties.getServerVersion() >= 17 && trackedPlayer.canSee(armorStand) && !WantedTags.isShowOwn())
            trackedPlayer.hideEntity(NaturalBounties.getInstance(), armorStand);
        // a fix for 1.16, the random is to help with performance
        if (NaturalBounties.getServerVersion() <= 16 && Math.random() <= 0.5) {
    ...
    public void spawn() {
        ...
        armorStand.getPersistentDataContainer().set(NaturalBounties.getNamespacedKey(), PersistentDataType.STRING, NaturalBounties.SESSION_KEY);
        if (NaturalBounties.getServerVersion() >= 17 && !WantedTags.isShowOwn())
            trackedPlayer.hideEntity(NaturalBounties.getInstance(), armorStand);
        lastLocation = armorStand.getLocation();
    }
