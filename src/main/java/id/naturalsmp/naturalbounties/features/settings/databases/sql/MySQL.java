package me.naturalsmp.NaturalBounties.features.settings.databases.sql;
...
import me.naturalsmp.NaturalBounties.data.player_data.OnlineRefund;
...
import me.naturalsmp.NaturalBounties.utils.SerializeInventory;
...
public class MySQL extends NaturalBountiesDatabase {
    ...
    public void addBounty(List<Bounty> bounties) {
        if (isConnected()) {
            try (PreparedStatement ps = getConnection().prepareStatement("INSERT INTO NaturalBounties(uuid, name, setter, suuid, amount, notified, created, whitelist, playtime, items, display) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);")) {
    ...
    public void removeBounty(List<Bounty> bounties) {
        if (isConnected()) {
            try (PreparedStatement ps = getConnection().prepareStatement("DELETE FROM NaturalBounties WHERE uuid = ? AND suuid = ? AND created = ?;")) {
                for (Bounty bounty : bounties) {
    ...
    public Whitelist decodeWhitelist(@Nullable String whitelist){
        ...
        for (String uuidString : split) {
            try {
                ...
                NaturalBounties.debugMessage(e.toString(), true);
            }
        }
    ...
    public @Nullable Bounty getBounty(UUID uuid) throws IOException{
        if (isConnected()) {
            try (PreparedStatement ps = getConnection().prepareStatement("SELECT * FROM `NaturalBounties` WHERE uuid = ?;")) {
                ps.setString(1, uuid.toString());
                try (ResultSet rs = ps.executeQuery()) {
    ...
    public void removeBounty(UUID uuid) {
        if (isConnected()) {
            try (PreparedStatement ps = getConnection().prepareStatement("DELETE FROM NaturalBounties WHERE uuid = ?")) {
                ps.setString(1, uuid.toString());
                ps.executeUpdate();
            } catch (SQLException e) {
                if (reconnect(e)) {
    ...
    public List<Bounty> getAllBounties(int sortType) throws IOException {
        if (isConnected()) {
            try (PreparedStatement ps = getConnection().prepareStatement("SELECT uuid, name, setter, suuid, amount, notified, created, whitelist, playtime, items, display FROM NaturalBounties;");
                ...
                while (resultSet.next()) {
                    ...
                    if (uuid != null) {
                        try {
                            ...
                            Bukkit.getLogger().warning("[NaturalBounties] Error decoding items from SQL");
    ...
    protected boolean reconnect(Exception e) {
        if (e instanceof SQLSyntaxErrorException) {
            Bukkit.getLogger().warning("[NaturalBounties] SQL Syntax Error! Please report this to NaturalDev.");
            Bukkit.getLogger().warning(e::toString);
    ...
    public void createTable(){
        try (PreparedStatement ps = getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS NaturalBounties" +
                "(" +
                ...
                ");");
             PreparedStatement checkAmount = getConnection().prepareStatement("select column_name,data_type from information_schema.columns where table_schema = ? and table_name = 'NaturalBounties' and column_name = 'amount';");
             PreparedStatement alterAmount = getConnection().prepareStatement("ALTER TABLE NaturalBounties MODIFY COLUMN amount FLOAT(53);");
             PreparedStatement checkWhitelist = getConnection().prepareStatement("SHOW COLUMNS FROM `NaturalBounties` LIKE 'whitelist';");
             PreparedStatement addWhitelist = getConnection().prepareStatement("ALTER TABLE NaturalBounties ADD whitelist VARCHAR(369);");
             PreparedStatement checkPlaytime = getConnection().prepareStatement("SHOW COLUMNS FROM `NaturalBounties` LIKE 'playtime';");
             PreparedStatement addPlaytime = getConnection().prepareStatement("ALTER TABLE NaturalBounties ADD playtime BIGINT DEFAULT 0 NOT NULL;");
             PreparedStatement checkItems = getConnection().prepareStatement("SHOW COLUMNS FROM `NaturalBounties` LIKE 'items';");
             PreparedStatement addItems = getConnection().prepareStatement("ALTER TABLE NaturalBounties ADD items BLOB;");
             PreparedStatement checkItemsType = getConnection().prepareStatement("select column_name,data_type from information_schema.columns where table_schema = ? and table_name = 'NaturalBounties' and column_name = 'items';");
             PreparedStatement alterItems = getConnection().prepareStatement("ALTER TABLE NaturalBounties MODIFY COLUMN items BLOB;");
             PreparedStatement checkDisplay = getConnection().prepareStatement("SHOW COLUMNS FROM `NaturalBounties` LIKE 'display';");
             PreparedStatement addDisplay = getConnection().prepareStatement("ALTER TABLE NaturalBounties ADD display FLOAT(53) DEFAULT -1 NOT NULL;");
             PreparedStatement checkTimeName = getConnection().prepareStatement("select column_name from information_schema.columns where table_schema = ? and table_name = 'NaturalBounties' and column_name = 'time';");
             PreparedStatement alterTimeName = getConnection().prepareStatement("ALTER TABLE NaturalBounties CHANGE time created BIGINT;")){
    ...
    private void addRefundBatch(List<OnlineRefund> refunds, PreparedStatement addRefund) throws SQLException {
        for (OnlineRefund onlineRefund : refunds) {
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Error encoding refund to SQL");
            }
        }
    }
    ...
    private OnlineRefund getRefundFromResult(ResultSet rs) throws SQLException {
        ...
        try (InputStream inputStream = rs.getBlob("refund").getBinaryStream()) {
            ...
            if (OnlineRefund.class.isAssignableFrom(clazz)) {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Invalid refund class: " + className);
            }
        } catch (IOException | ReflectiveOperationException e) {
            Bukkit.getLogger().warning("[NaturalBounties] Error decoding refund: " + e.getMessage());
        }
        return null;
    }
    ...
    public void notifyBounty(UUID uuid) {
        if (isConnected()) {
            try (PreparedStatement ps = getConnection().prepareStatement("UPDATE NaturalBounties SET notified = 1 WHERE uuid = ?;")) {
                ps.setString(1, uuid.toString());
