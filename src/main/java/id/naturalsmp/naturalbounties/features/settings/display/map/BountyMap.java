...
import me.naturalsmp.NaturalBounties.ui.gui.CompatabilityUtils;
import org.bukkit.*;
...
import static me.naturalsmp.NaturalBounties.utils.BountyManager.getBounty;
import static me.naturalsmp.NaturalBounties.utils.BountyManager.hasBounty;
...
public class BountyMap implements Listener {
    ...
    public static void initialize(Plugin plugin){
        ...
        posterDirectory = new File(NaturalBounties.getInstance().getDataFolder() + File.separator + "posters");
        ...
        if (!bountyPosterFile.exists())
            NaturalBounties.getInstance().saveResource("posters/bounty poster.png", false);
        if (!new File(posterDirectory + File.separator + "poster template.png").exists())
            NaturalBounties.getInstance().saveResource("posters/poster template.png", false);
        File deadBountyFile = new File(posterDirectory + File.separator + "dead bounty.png");
        if (!deadBountyFile.exists())
            NaturalBounties.getInstance().saveResource("posters/dead bounty.png", false);
        if (!new File(posterDirectory + File.separator + "READ_ME.txt").exists())
            NaturalBounties.getInstance().saveResource("posters/READ_ME.txt", false);
        ...
        try {
            ...
            Bukkit.getLogger().warning("[NaturalBounties] Could not load a poster image.");
    ...
    public static void loadConfiguration(ConfigurationSection config) {
        ...
        hologramUpdateTask = NaturalBounties.getServerImplementation().global().runAtFixedRate(() -> {
            ...
            for (World world : Bukkit.getWorlds()) {
                for (Chunk chunk : world.getLoadedChunks()) {
                    for (Entity entity : chunk.getEntities()) {
                        if (entity.getType() == EntityType.ITEM_FRAME || (NaturalBounties.getServerVersion() >= 17 && entity.getType() == EntityType.GLOW_ITEM_FRAME)) {
    ...
    public void onMapHold(PlayerItemHeldEvent event) {
        ...
        UUID uuid = UUID.fromString(Objects.requireNonNull(mapMeta.getPersistentDataContainer().get(NaturalBounties.getNamespacedKey(), PersistentDataType.STRING)));
    ...
    public void onPrepareCraft(PrepareItemCraftEvent event) {
        ...
        if (NaturalBounties.isPaused() || !enabled)
            return;
        if (!craftPoster) {
            boolean hasPerm = false;
            for (HumanEntity humanEntity : event.getViewers()) {
                if (humanEntity.hasPermission("NaturalBounties.craftposter")){
                    hasPerm = true;
                    break;
                }
            }
        ...
        if (meta.getOwningPlayer() == null) {
            NaturalBounties.debugMessage("Player head in crafting matrix has a null owner!", false);
            return;
        }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        if ((!craftPoster && !event.getWhoClicked().hasPermission("NaturalBounties.craftposter")) || !(event.getInventory() instanceof CraftingInventory inventory) || NaturalBounties.isPaused() || !enabled)
            return;
    ...
    public void onPlayerItemDrop(PlayerDropItemEvent event) {
        if ((!washPoster && !event.getPlayer().hasPermission("NaturalBounties.washposter")) || !enabled || !isPoster(event.getItemDrop().getItemStack()) || NaturalBounties.isPaused())
            return;

        NaturalBounties.getServerImplementation().entity(event.getItemDrop()).runDelayed(() -> {
            if (!event.getItemDrop().isValid())
                return;
            ItemStack map = new ItemStack(Material.MAP, event.getItemDrop().getItemStack().getAmount());
            if ((NaturalBounties.getServerVersion() < 17 && event.getItemDrop().getLocation().getBlock().getType() == Material.CAULDRON)
                    || (NaturalBounties.getServerVersion() >= 17 && event.getItemDrop().getLocation().getBlock().getType() == Material.WATER_CAULDRON)) {
    ...
    public static boolean isPoster(ItemStack itemStack) {
        ...
        return meta.getPersistentDataContainer().has(NaturalBounties.getNamespacedKey(), PersistentDataType.STRING);
    ...
    public static void loadFont() {
        if (new File(posterDirectory + File.separator + "playerfont.ttf").exists()) {
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Detected a custom font, but an error occurred when reading it! This may be because of a corrupted font file or an invalid java font configuration file.");
    ...
    public static ItemStack getMap(UUID uuid, double displayBounty, long updateTime) {
        ...
        try {
            ...
            NaturalBounties.debugMessage("Error setting map view for poster!", true);
        ...
        meta.getPersistentDataContainer().set(NaturalBounties.getNamespacedKey(), PersistentDataType.STRING, uuid.toString());
