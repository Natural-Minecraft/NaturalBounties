...
import static me.naturalsmp.NaturalBounties.features.LanguageOptions.getListMessage;
...
public class BountyHunt {
    ...
    public static void loadConfiguration(ConfigurationSection config) {
        if (huntKey == null)
            huntKey = new NamespacedKey(NaturalBounties.getInstance(), "hunt");
    ...
    public static boolean executeHuntCommand(CommandSender sender, String[] args, boolean silent, boolean adminPermission, Player parser) {
        ...
        if (args.length == 1) {
            if (!hasPermission(sender, adminPermission, "NaturalBounties.hunt.start")) {
        ...
        if (!hasPermission(sender, adminPermission, "NaturalBounties.hunt.participate")) {
            return failNoPermission(sender, silent, parser);
        }
