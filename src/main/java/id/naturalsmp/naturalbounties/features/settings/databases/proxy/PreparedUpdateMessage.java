...
public class PreparedUpdateMessage {
    ...
    public PreparedUpdateMessage(String channel, List<String[]> messages, long id) {
        ...
        try {
            ...
            NaturalBounties.getInstance().getLogger().log(Level.WARNING, "Failed to prepare proxy message", e);
    ...
    public void sendMessage(boolean delay) {
        ...
        if (!NaturalBounties.getInstance().isEnabled()) {
            ...
            NaturalBounties.getInstance().getLogger().log(Level.WARNING, "Plugin shutting down in the middle of a proxy message.");
            ProxyMessaging.onFailProxyMessage(id);
            return;
        }
        sending = true;
        if (delay) {
            NaturalBounties.getServerImplementation().global().runDelayed(this::executeMessage, 5);
        } else {
            executeMessage();
        }
    ...
    private void executeMessage() {
        if (!Bukkit.getOnlinePlayers().isEmpty() && NaturalBounties.getInstance().isEnabled() && ProxyDatabase.isEnabled() && !canceled) {
            ...
            NaturalBounties.debugMessage("Failed to send proxy update.", true);
            ProxyMessaging.onFailProxyMessage(id);
            sending = false;
        }
    }
