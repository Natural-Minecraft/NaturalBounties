...
public class BountyClaimRequirements {
    ...
    public static boolean canClaim(Player player, Player killer) {
        ...
        if ((ConfigOptions.isWorldFilter() && !ConfigOptions.getWorldFilterNames().contains(player.getWorld().getName()) || (!ConfigOptions.isWorldFilter() && ConfigOptions.getWorldFilterNames().contains(player.getWorld().getName())))) {
            NaturalBounties.debugMessage("Bounty claim canceled due to a filtered world.", false);
            return false;
        }
        // check for teams
        if (betterTeamsEnabled) {
            ...
            if ((!btClaim && betterTeamsClass.onSameTeam(player, killer)) || (!btAllies && betterTeamsClass.areAllies(player, killer))) {
                NaturalBounties.debugMessage("Bounty claim canceled due to a BetterTeams team.", false);
                return false;
            }
        }
        if (townyAdvancedEnabled) {
            ...
            if ((!townyNation && townyAdvancedClass.inSameNation(player, killer)) || (!townyTown && townyAdvancedClass.inSameTown(player, killer)) || (!townyAllies && townyAdvancedClass.areNationsAllied(player, killer))) {
                NaturalBounties.debugMessage("Bounty claim canceled due to a TownyAdvanced town.", false);
                return false;
            }
        }
        if (kingdomsXEnabled) {
            ...
            if ((!kingdomsXNation && kingdomsXClass.inSameNation(player, killer)) || (!kingdomsXKingdom && kingdomsXClass.inSameKingdom(player,killer)) || (!kingdomsXNationAllies && kingdomsXClass.getNationRelation(player, killer) == 2) || (!kingdomsXKingdomAllies && kingdomsXClass.getKingdomRelation(player, killer) == 2)) {
                NaturalBounties.debugMessage("Bounty claim canceled due to a KingdomsX kingdom.", false);
                return false;
            }
        }
        if (landsEnabled) {
            ...
            if ((!landsNation && landsClass.shareNation(player, killer)) || (!landsLand && landsClass.shareLand(player,killer)) || (!landsNationAllies && landsClass.areNationsAllied(player, killer)) || (!landsLandAllies && landsClass.areLandsAllied(player, killer)) || !landsClass.canClaim(killer, player.getLocation())) {
                NaturalBounties.debugMessage("Bounty claim canceled due to a Lands plugin land.", false);
                return false;
            }
        }
        if (!scoreboardTeamClaim && Bukkit.getScoreboardManager() != null) {
                ...
                for (Team team : sb.getTeams()) {
                    if (team.hasEntry(player.getDisplayName()) && team.hasEntry(killer.getDisplayName())) {
                        NaturalBounties.debugMessage("Bounty claim canceled due to a scoreboard team.", false);
                        return false;
                    }
                }
            }
            ...
            for (String placeholder : teamsPlaceholder) {
                if (!placeholder.isBlank()) {
                    if (!placeholder.contains("%")) {
                        NaturalBounties.debugMessage("Warning: 'placeholder' option in the integrations.yml file does not include the '%' symbol.", true);
                    }
                    ...
                    if (parsed1 != null && !excludedPlaceholderResults.contains(parsed1) && parsed1.equals(parsed2)) {
                        NaturalBounties.debugMessage("Bounty claim canceled due to a PlaceholderAPI placeholder match in integrations configuration: \"" + placeholder + "\" = \"" + parsed1 + "\"", false);
                        return false;
                    }
                }
            }
        ...
        if (ConfigOptions.getIntegrations().isWorldGuardEnabled()) {
            if (!WorldGuardClass.canClaim(killer, player.getLocation())) {
                NaturalBounties.debugMessage("Bounty claim canceled due to a WorldGuard flag.", false);
                return false;
            }
        }
        if (factionsEnabled) {
            try {
                ...
                if ((!factionsFaction && sabreFactionsClass.inSameFaction(player, killer)) || (!factionsAlly && sabreFactionsClass.areFactionsAllied(player, killer))) {
                    NaturalBounties.debugMessage("Bounty claim canceled due to a SaberFaction faction.", false);
                    return false;
                }
            ...
            try {
                ...
                if ((!factionsFaction && factionsUUIDClass.inSameFaction(player, killer)) || (!factionsAlly && factionsUUIDClass.areFactionsAllied(player, killer))) {
                    NaturalBounties.debugMessage("Bounty claim canceled due to a FactionsUUID faction.", false);
                    return false;
                }
            } catch (NoSuchMethodError ignored) {
                // not using this specific factions plugin
        ...
        if (superiorSkyblockEnabled && !superiorSkyblockIslandMember && SuperiorSkyblockClass.onSameIsland(player, killer)) {
            NaturalBounties.debugMessage("Bounty claim canceled due to a SuperiorSkyblock2 island.", false);
            return false;
        }
        ...
        if (ConfigOptions.getIntegrations().isDuelsEnabled()
            ...
            NaturalBounties.debugMessage("Bounty claim canceled due to a Duels plugin duel.", false);
            return false;
        }
        ...
        if (simpleClansEnabled
            ...
            NaturalBounties.debugMessage("Bounty claim canceled due to a SimpleClans clan.", false);
            return false;
        }
        ...
        && ((!konquestKingdom && KonquestClass.isInSameKingdom(player, killer))
            ...
            NaturalBounties.debugMessage("Bounty claim canceled due to a Konquest kingdom.", false);
            return false;
        }
        // check IP
        if (!sameIPClaim && player.getAddress() != null && killer.getAddress() != null && player.getAddress().getAddress().getHostAddress().equals(killer.getAddress().getAddress().getHostAddress())) {
            NaturalBounties.debugMessage("Bounty claim canceled due to a matching IP address.", false);
            return false;
        }
        ...
        if (minRespawnDistance >= 0
            ...
            NaturalBounties.debugMessage("Bounty claim canceled due to a close respawn distance.", false);
            return false;
        }
        if (BountyHunt.isPauseOtherBounties() && !BountyHunt.getHunts().isEmpty()) {
            NaturalBounties.debugMessage("Bounty claim canceled due to an active bounty hunt.", false);
            return false;
        }
