...
import me.naturalsmp.NaturalBounties.ui.iso_renderer.IsometricRenderer;
...
public class SkinManager {
    ...
    public static void loadConfiguration(ConfigurationSection config) {
        try {
            ...
            NaturalBounties.getInstance().getLogger().warning("Failed to load missing skin texture.");
        }
    }
    ...
    public static void refreshSkinRequests() {
        ...
        try {
            ...
            Bukkit.getLogger().warning("[NaturalBounties] Failed to refresh skin cache.");
            Bukkit.getLogger().warning("[NaturalBounties] Skins are currently being processed.");
            Bukkit.getLogger().warning("[NaturalBounties] Please try again.");
        }
    }

    public static void saveSkin(UUID uuid, PlayerSkin playerSkin) {
        savedSkins.put(uuid, playerSkin);
        NaturalBounties.debugMessage("Saved player skin -> " + uuid, false);
    }
    ...
    public static void saveSkin(UUID uuid) {
        ...
        NaturalBounties.debugMessage("Attempting to save skin for: " + uuid, false);
        if (savedSkins.containsKey(uuid))
            return;
    ...
    public static void requestSkin(UUID uuid, boolean firstAttempt) {
        if (isSafeToRequest()) {
            ...
            NaturalBounties.debugMessage("Too many requests to save skins! Delaying some... ", false);
            requestLater(uuid);
        }
    }
    ...
    protected static int getRecentRequestCount() {
        if (System.currentTimeMillis() - lastLimitCheck < 5000) {
            ...
            NaturalBounties.getServerImplementation().global().run(() -> rateLimit.removeIf(l -> l < minKeepTime));
    ...
    protected static void requestLater(UUID uuid) {
        if (queuedRequests.isEmpty() && !rateLimit.isEmpty()) {
            ...
            NaturalBounties.getServerImplementation().async().runDelayed(() -> {
    ...
    public static void incrementMojangRate() {
        NaturalBounties.getServerImplementation().global().run(() -> rateLimit.add(System.currentTimeMillis()));
    }
    ...
    public static BufferedImage getPlayerFace(UUID uuid) {
        ...
        try {
            ...
            NaturalBounties.debugMessage("Error reading texture url for rendering player face.\n" + e, true);
    ...
    public static BufferedImage getIsometricFace(UUID uuid) {
        ...
        try {
            ...
            NaturalBounties.debugMessage("Error reading texture url for rendering isometric head.\n" + e, true);
        }
        return null;
    }
    ...
    public static URL getTextureURL(String texture) {
        try {
            ...
            // too many requests
            NaturalBounties.debugMessage("Error getting texture url: " + texture, true);
            return null;
        }
    }
}
...
class SkinResponseHandler {
    ...
    public static void closeClient(){
        if (client != null) {
            client.cancel();
            client = null;
            NaturalBounties.debugMessage("Http client has been closed.", false);
        }
    }
    ...
    private static void createClient() {
        if (client != null)
            return;
        NaturalBounties.debugMessage("Creating new skin request client.", false);
        client = NaturalBounties.getServerImplementation().async().runAtFixedRate(() -> {
            ...
            try (final CloseableHttpClient httpclient = HttpClients.custom().disableAutomaticRetries().build()) {
                ...
                NaturalBounties.debugMessage("Failed to request skin: " + uuid, true);
    ...
    private static @Nullable PlayerSkin requestPlayerSkin(SkinRequestType skinRequestType, CloseableHttpClient httpClient) throws IOException {
        ...
        switch (skinRequestType.skinType()) {
            ...
            case BEDROCK -> {
                return saveGeyserSkin(httpClient, NaturalBounties.getXuid(uuid));
            }
            case USERNAME -> {
    ...
    protected static void webRequestSkin(UUID uuid, boolean firstAttempt){
        ...
        } else if (NaturalBounties.isBedrockPlayer(uuid)) {
    ...
    private static PlayerSkin saveGeyserSkin(CloseableHttpClient httpClient, String xUID) throws IOException {
        NaturalBounties.debugMessage("Attempting to get bedrock skin from xuid: " + xUID, false);

        final HttpGet request = new HttpGet("https://api.geysermc.org/v2/skin/" + xUID);

        return httpClient.execute(request, response -> {
            StatusLine statusLine = new StatusLine(response);
            NaturalBounties.debugMessage(request + "->" + new StatusLine(response), false);
    ...
    private static PlayerSkin saveJavaSkin(CloseableHttpClient httpClient, UUID requestUUID, boolean tryNamed) throws IOException {
        ...
        return httpClient.execute(request, response -> {
            StatusLine statusLine = new StatusLine(response);
            NaturalBounties.debugMessage(request + "->" + statusLine, false);
            if (statusLine.getStatusCode() == 200 && response.getEntity() != null) {
                String text = EntityUtils.toString(response.getEntity());
                JsonObject input = new JsonParser().parse(text).getAsJsonObject();
                ...
                if (tryNamed) {
                    NaturalBounties.debugMessage("Saving named skin", false);
