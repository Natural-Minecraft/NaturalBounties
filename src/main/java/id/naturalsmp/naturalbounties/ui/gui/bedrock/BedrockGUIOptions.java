package me.naturalsmp.NaturalBounties.ui.gui.bedrock;
...
import me.naturalsmp.NaturalBounties.utils.tasks.OpenBedrockGUI;
...
import static me.naturalsmp.NaturalBounties.ui.gui.GUIOptions.getPageType;
...
public class BedrockGUIOptions {
    ...
    public BedrockGUIOptions(ConfigurationSection settings, ConfigurationSection customItems) {
        ...
        if (settings.isString("override-type"))
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Invalid GUI override type in bedrock-gui \"" + name + "\": " + settings.getString("override-type"));
            }
            ...
            for (String key : Objects.requireNonNull(settings.getConfigurationSection("components")).getKeys(false)) {
                ...
                if (componentID != null)
                    if (customItems.isConfigurationSection(componentID)) {
                        ...
                        Bukkit.getLogger().warning("[NaturalBounties] Could not find custom item \"" + componentID + "\" in bedrock-gui.yml");
                    }
            }
    ...
    public void openInventory(Player player, long page, long maxPage, List<DisplayItem> displayItems, String title, Object[] data) {
        ...
        openBedrockGUI.setTaskImplementation(NaturalBounties.getServerImplementation().async().runAtFixedRate(openBedrockGUI, 1, 4));
    ...
    public void doClickActions(Player player, SimpleFormResponse simpleFormResponse, List<GUIComponent> usedGUIComponents) {
        if (simpleFormResponse.clickedButtonId() >= usedGUIComponents.size()) {
            Bukkit.getLogger().warning(() -> "[NaturalBounties] This is a bug! User clicked an unregistered button in " + type + " bedrock gui.");
            return;
        }
    ...
    public void doClickActions(Player player, CustomFormResponse customFormResponse, List<GUIComponent> usedGUIComponents) {
        ...
        for (GUIComponent component : usedGUIComponents) {
            if (!customFormResponse.hasNext()) {
                Bukkit.getLogger().warning("[NaturalBounties] This is a bug! Form content length does not match records for " + type + " bedrock gui.");
                break;
            }
            ...
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] This is a bug! Custom form response component does not match up with the recorded component in " + type + " bedrock gui.");
