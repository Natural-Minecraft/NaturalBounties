...
import me.naturalsmp.NaturalBounties.ui.HeadFetcher;
    ...
    public GUIOptions(ConfigurationSection settings) {
        ...
        if (settings.isString("inventory-type")) {
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Invalid inventory type \"" + settings.getString("inventory-type") + "\" for gui: " + type);
                ...
                if (GUI.getCustomItems().containsKey(item)) {
                    ...
                    for (int i : slots) {
                        if (i >= customItems.length) {
                            Bukkit.getLogger().warning("[NaturalBounties] Error loading custom item in GUI: " + type);
