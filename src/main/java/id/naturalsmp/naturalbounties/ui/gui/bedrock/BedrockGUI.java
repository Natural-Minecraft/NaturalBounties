...
import me.naturalsmp.NaturalBounties.ui.gui.display_items.DisplayItem;
...
public class BedrockGUI {
    ...
    public static void reloadOptions(){
        if (!getFile().exists())
            NaturalBounties.getInstance().saveResource(FILE_NAME, false);
        YamlConfiguration configuration = YamlConfiguration.loadConfiguration(getFile());
        if (configuration.getKeys(true).size() <= 2) {
            Bukkit.getLogger().severe("[NaturalBounties] Loaded an empty configuration for the bedrock-gui.yml file. Fix the YAML formatting errors, or the GUI may not work!\nFor more information on YAML formatting, see here: https://spacelift.io/blog/yaml");
            if (NaturalBounties.getInstance().getResource(FILE_NAME) != null) {
                configuration = YamlConfiguration.loadConfiguration(new InputStreamReader(Objects.requireNonNull(NaturalBounties.getInstance().getResource(FILE_NAME))));
    ...
    private static void loadGUI(YamlConfiguration configuration) {
        ...
        if (changes) {
            try {
                ...
                Bukkit.getLogger().warning("[NaturalBounties] Error trying to save updates to bedrock-gui.yml.");
            }
        }
    ...
    private static File getFile() {
        return new File(NaturalBounties.getInstance().getDataFolder() + File.separator + FILE_NAME);
    }
