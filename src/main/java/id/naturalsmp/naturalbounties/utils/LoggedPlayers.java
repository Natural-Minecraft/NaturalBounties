...
public class LoggedPlayers {
    ...
    public static void loadPlayerData() {
        ...
        if (!duplicateUUIDs.isEmpty()) {
            Bukkit.getLogger().log(Level.WARNING, "[NaturalBounties] There are {0} logged players with no associated name.", duplicateUUIDs.size());
            NaturalBounties.debugMessage(duplicateUUIDs.toString(), false);
        }
    ...
    private static void webRequestPlayerName(@NotNull UUID uuid, PlayerData playerData) {
        if (uuid.version() == 4 /* check if online player */) {
            ...
            httpPool.requestPlayerNameAsync(uuid, new HttpSyncPool.ResponseHandler())
                    ...
                    .exceptionally(ex -> {
                        NaturalBounties.debugMessage("Failed to get player name for " + uuid + ".", true);
                        return null;
                    });
        }
    }
