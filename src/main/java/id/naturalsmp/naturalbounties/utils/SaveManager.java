...
import me.naturalsmp.NaturalBounties.data.player_data.PlayerDataAdapter;
...
import me.naturalsmp.NaturalBounties.features.settings.databases.proxy.PreparedUpdateMessage;
...
import me.naturalsmp.NaturalBounties.features.settings.display.BountyHuntTypeAdapter;
...
import me.naturalsmp.NaturalBounties.features.settings.display.map.BountyBoardTypeAdapter;
...
public class SaveManager {
    ...
    public static void save(Plugin plugin) throws IOException {
        if (saveLock) {
            NaturalBounties.debugMessage("Save already in progress, skipping...", false);
            return;
        }
        saveLock = true;
        NaturalBounties.debugMessage("Saving...", false);

        try {
            File dataDirectory = new File(plugin.getDataFolder() + File.separator + "data");
            if (dataDirectory.mkdir())
                NaturalBounties.debugMessage("Created new data directory", false);
    ...
    private static void saveBounties(File dataDirectory) throws IOException {
        ...
        if (bountiesFile.createNewFile()) {
            NaturalBounties.debugMessage("Created a new bounties.json file.", false);
        }
        try (JsonWriter writer = new JsonWriter(new FileWriter(bountiesFile))) {
            ...
            NaturalBounties.debugMessage("Saving " + bounties.size() + " bounties.", false);
    ...
    private static void saveStats(File dataDirectory) throws IOException {
        ...
        if (statsFile.createNewFile()) {
            NaturalBounties.debugMessage("Created a new player_stats.json file.", false);
        }
        try (JsonWriter writer = new JsonWriter(new FileWriter(statsFile))) {
            ...
            NaturalBounties.debugMessage("Saving " + playerStatMap.size() + " stats.", false);
    ...
    private static void savePlayerData(File dataDirectory) throws IOException {
        ...
        if (playerDataFile.createNewFile()) {
            NaturalBounties.debugMessage("Created a new player_data.json file.", false);
        }
        ...
        try (JsonWriter writer = new JsonWriter(new FileWriter(playerDataFile))) {
            ...
            NaturalBounties.debugMessage("Saving " + playerDataList.size() + " player data.", false);
            ...
            writer.name("paused").value(NaturalBounties.isPaused());
        ...
        try (JsonReader reader = new JsonReader(new FileReader(playerDataFile))) {
            ...
            while (reader.hasNext()) {
                ...
                switch (name) {
                    ...
                    case "paused" -> NaturalBounties.setPaused(reader.nextBoolean());
    ...
    public static void saveUnsentProxyMessages(File dataDirectory) throws IOException {
        ...
        if (!saveFile.exists()) {
            if (saveFile.createNewFile())
                NaturalBounties.getInstance().getLogger().info("Created a new proxy_message_cache.bin file.");
        }
    ...
    public static List<byte[]> readUnsentProxyMessages(File dataDirectory) throws IOException{
        ...
        if (Files.deleteIfExists(saveFile.toPath()))
            NaturalBounties.debugMessage("Deleted proxy_message_cache.bin.", false);
