...
public class Tutorial {
    ...
    private static void sendPage(@NotNull CommandSender sender, int page) {
        NaturalBounties.debugMessage("Sending Tutorial Page " + page + " to " + sender.getName(), false);
        if (pages.size() < page)
            return;
        Player parser = sender instanceof Player player ? player : null;
        List<String> get = pages.get(page - 1);
        NaturalBounties.debugMessage("Tutorial (" + page + ":" + sender.getName() + ") Sending Title.", false);
        sender.sendMessage(parse(getMessage("help.title"), parser));
        NaturalBounties.debugMessage("Tutorial (" + page + ":" + sender.getName() + ") Sending Contents.", false);
        for (String line : get) {
            TextComponent[] components = parseLine(line, parser);
            sender.spigot().sendMessage(components);
        }
        NaturalBounties.debugMessage("Tutorial (" + page + ":" + sender.getName() + ") Sending Page Line.", false);
