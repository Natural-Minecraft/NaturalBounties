...
import me.naturalsmp.NaturalBounties.data.player_data.*;
import me.naturalsmp.NaturalBounties.features.ActionCommands;
import me.naturalsmp.NaturalBounties.features.settings.databases.AsyncDatabaseWrapper;
import me.naturalsmp.NaturalBounties.features.settings.databases.LocalData;
import me.naturalsmp.NaturalBounties.features.settings.databases.NaturalBountiesDatabase;
import me.naturalsmp.NaturalBounties.features.settings.databases.TempDatabase;
...
import me.naturalsmp.NaturalBounties.features.settings.databases.redis.RedisConnection;
import me.naturalsmp.NaturalBounties.features.settings.databases.sql.MySQL;
...
public class DataManager {
    ...
    public static void connectProxy(List<Bounty> bounties, Map<UUID, PlayerStat> playerStatMap, List<PlayerData> playerDataMap) {
        ...
        NaturalBounties.getServerImplementation().async().runNow(task -> {
    ...
    public static void loadDatabaseConfig(ConfigurationSection configuration, Plugin plugin) {
        ...
        if (configuration.isSet("auto-connect-interval")) {
            ...
            if (autoConnectInterval > 0) {
                autoReconnectTask = NaturalBounties.getServerImplementation().async().runAtFixedRate(DataManager::tryDatabaseConnections, 120, autoConnectInterval * 20);
            }
        }
    ...
    private static void loadNewDatabaseConfig(ConfigurationSection configuration, String databaseName) throws IllegalArgumentException {
        ...
        NaturalBountiesDatabase database;
        try {
            switch (type.toUpperCase()) {
                case "SQL" -> database = new MySQL(NaturalBounties.getInstance(), databaseName);
                case "REDIS" -> database = new RedisConnection(NaturalBounties.getInstance(), databaseName);
                case "PROXY" -> database = new ProxyDatabase(NaturalBounties.getInstance(), databaseName);
            ...
            NaturalBounties.debugMessage("One or more dependencies could not be downloaded to use the database: " + databaseName + " (" + type + ")", true);
        }
    }
    ...
    private static void loadOldData() {
        File bounties = new File(NaturalBounties.getInstance().getDataFolder() + File.separator + "bounties.yml");
        if (!bounties.exists())
            return;
        ...
        try {
            ...
            for (Map.Entry<UUID, Boolean[]> entry : immunityMap.entrySet()) {
                ...
                if (configuration.isBoolean("paused"))
                    NaturalBounties.setPaused(configuration.getBoolean("paused"));
                ...
                if (configuration.isConfigurationSection("database-sync-times")) {
                    for (String key : Objects.requireNonNull(configuration.getConfigurationSection("database-sync-times")).getKeys(false)) {
                        for (NaturalBountiesDatabase database : databases) {
                ...
                if (configuration.isList("wanted-tags")) {
                    ...
                    NaturalBounties.getServerImplementation().global().runDelayed(task -> {
     ...
     * @return The set server ID for NaturalBounties.
     */
    ...
    public static void handleRefund(PlayerData playerData) {
        ...
        NaturalBounties.getServerImplementation().global().run(() -> {
            ...
            if (player != null && !playerData.getRefund().isEmpty()) {
                NaturalBounties.debugMessage("Giving " + playerData.getPlayerName() + " a refund.", false);
                ...
                NaturalBounties.getServerImplementation().async().runNow(() -> syncPlayerData(playerData.getUuid(), null));
            }
        });
    ...
    public static List<PlayerData> getAllPlayerData() {
        try {
            ...
            // comes from the NaturalBountiesDatabase interface
   ...
   public static void removeSimilarSetters(List<Setter> masterSetterList, List<Setter> setterList) {
    ...
    public static void removeSetters(@NotNull Bounty bounty, List<Setter> setters) {
        ...
        if (bountyCopy.getTotalDisplayBounty() < WantedTags.getMinWanted()) {
            // remove bounty tag
            NaturalBounties.getServerImplementation().global().run(() -> WantedTags.removeWantedTag(bounty.getUUID()));
            NaturalBounties.debugMessage("Removed wanted tag.", false);
        }
     ...
     */
    public static void databaseConnect(NaturalBountiesDatabase database) {
    ...
    public static void getAndSyncDatabase(NaturalBountiesDatabase database) {
        ...
        try {
            ...
            if (playerDataLoadTime > 1000) {
                plugin.getLogger().warning("[NaturalBounties] Loading " + playerDataMap.size() + " player data records took " + playerDataLoadTime + "ms from " + database.getName() + ". Consider increasing refresh-interval if this happens frequently.");
            }
        } catch (IOException e) {
            plugin.getLogger().warning("Error while trying to connect to " + database.getName() + " (bad connection)");
            Arrays.stream(e.getStackTrace()).forEach(stackTraceElement -> NaturalBounties.debugMessage(stackTraceElement.toString(), false));
        ...
        if (totalTime > 2000) {
            plugin.getLogger().warning("[NaturalBounties] Database sync for " + database.getName() + " took " + totalTime + "ms. This may cause server lag.");
        }
    }
    ...
    private static synchronized void syncDatabase(NaturalBountiesDatabase database, List<Bounty> databaseBounties, Map<UUID, PlayerStat> databaseStats, List<PlayerData> databasePlayerData) {
        NaturalBounties.debugMessage("Synchronizing database \"" + database.getName() + "\" with " + databaseBounties.size() + " bounties and "  + databaseStats.size() + " stat records.", false);
    ...
    private static void syncConsistentData(NaturalBountiesDatabase database, List<Bounty> databaseBounties, Map<UUID, PlayerStat> databaseStats, List<PlayerData> databasePlayerData, List<Bounty> localBounties, long lastSyncTime, AsyncDatabaseWrapper databaseWrapper, List<PlayerData> localPlayerData, Map<UUID, PlayerStat> localStats) {
        ...
        NaturalBounties.debugMessage("Since last sync: databaseAdded=" + databaseAdded.size() + " databaseRemoved=" + databaseRemoved.size() + " localAdded=" + localAdded.size() + " localRemoved=" + localRemoved.size(), false);
        ...
        if (syncTime > 1000 && (localPlayerData.size() > 10000 || databasePlayerData.size() > 10000)) {
            NaturalBounties.debugMessage("Player data sync took " + syncTime + "ms for " + localPlayerData.size() + " local and " + databasePlayerData.size() + " database records. Synced: " + syncedPlayerData.size(), false);
        }
        // apply consistency changes
        localData.addPlayerData(syncedPlayerData);
        databaseWrapper.addPlayerData(syncedPlayerData);

        NaturalBounties.debugMessage("Player Data synced:" + syncedPlayerData.size() , false);
