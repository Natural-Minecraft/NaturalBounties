...
import me.naturalsmp.NaturalBounties.ui.QueuedHead;
...
public class HeadLoader extends CancelableTask{
    ...
    public void run() {
        ...
        // update inventory
        NaturalBounties.getServerImplementation().entity(player).run(() -> {
            if (player.isOnline() && GUI.playerInfo.containsKey(player.getUniqueId())) {
                ...
                maxRequests = 0;
                NaturalBounties.debugMessage("Player exited GUI while loading player heads.", false);
            }
        });
        ...
        if (maxRequests <= 0) {
            ...
            if (NaturalBounties.isDebug()) {
                ...
                for (QueuedHead queuedHead : heads) {
                    if (fetchedHeads[i] == null) {
                        NaturalBounties.debugMessage("[NaturalBountiesDebug] Timed out loading skin for " + LoggedPlayers.getPlayerName(queuedHead.uuid()), true);
                    }
                    i++;
                }
            }
    ...
    private ItemStack copyItemText(ItemStack from, ItemStack to) {
        ...
        if (NaturalBounties.isAboveVersion(21, 3)) {
