...
public class LoadPlaytimeTask extends CancelableTask{
    ...
    public void run() {
        ...
        if (index >= allUsers.size()) {
            cancel();
            NaturalBounties.getInstance().getLogger().log(Level.INFO, "Loaded playtime for {0} players. Took {1} ms of compute over {2} ticks ({3} skipped).", new Object[]{playersChecked, totalTimeElapsed, tickCount, skippedTicks});
        }
    }
}
