...
import me.naturalsmp.NaturalBounties.features.webhook.DiscordWebhook;
import me.naturalsmp.NaturalBounties.features.webhook.Webhook;
import me.naturalsmp.NaturalBounties.features.webhook.WebhookField;
...
public class WebhookBuilder extends CancelableTask{
    ...
    public void run() {
        ...
        if ((!avatarLoaded || !imageLoaded) && maxRequests > 0) {
            ...
            maxRequests--;
            if (maxRequests <= 0) {
                NaturalBounties.debugMessage("Timed out loading skin for " + LoggedPlayers.getPlayerName(avatarUUID) + " or " + LoggedPlayers.getPlayerName(imageUUID), true);
            }
            return;
        }
        this.cancel();
        ...
        try {
            ...
            NaturalBounties.debugMessage("Could not send a discord webhook!", true);
