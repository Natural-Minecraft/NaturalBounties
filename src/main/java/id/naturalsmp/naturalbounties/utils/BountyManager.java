...
import me.naturalsmp.NaturalBounties.bounty_events.BountyClaimEvent;
import me.naturalsmp.NaturalBounties.bounty_events.BountySetEvent;
import me.naturalsmp.NaturalBounties.data.player_data.AmountRefund;
import me.naturalsmp.NaturalBounties.data.player_data.ItemRefund;
import me.naturalsmp.NaturalBounties.data.player_data.PlayerData;
import me.naturalsmp.NaturalBounties.data.player_data.RewardHead;
import me.naturalsmp.NaturalBounties.features.settings.auto_bounties.BigBounty;
...
import me.naturalsmp.NaturalBounties.ui.Head;
...
import me.naturalsmp.NaturalBounties.features.settings.auto_bounties.TrickleBounties;
import me.naturalsmp.NaturalBounties.utils.tasks.BroadcastTask;
import me.naturalsmp.NaturalBounties.utils.tasks.DelayedReward;
...
import me.naturalsmp.NaturalBounties.features.settings.integrations.external_api.MMOLibClass;
...
public class BountyManager {
    ...
    public static void addBounty(Player setter, OfflinePlayer receiver, double amount, List<ItemStack> items, Whitelist whitelist) {
        ...
        broadcastTask.setTaskImplementation(NaturalBounties.getServerImplementation().async().runAtFixedRate(broadcastTask,1,4));
    ...
    public static void refundPlayer(UUID uuid, double amount, List<ItemStack> items, String reason) {
        ...
        if (amount > 0) {
            if (NumberFormatting.isVaultEnabled() && !NumberFormatting.isOverrideVault()) {
                if (!NumberFormatting.getVaultClass().deposit(player, amount)) {
                    Bukkit.getLogger().warning("[NaturalBounties] Error depositing currency with vault! Will retry when player joins next.");
                    addRefund(uuid, amount, reason);
                    refund = true;
                }
            } else {
                if (player != null && NaturalBounties.getInstance().isEnabled()) {
            ...
            // sync player data with the most up-to-date database
        {
            if (NaturalBounties.getInstance().isEnabled()) {
                NaturalBounties.getServerImplementation().async().runNow(() -> DataManager.syncPlayerData(uuid, null));
            } else {
    ...
    public static void checkDelayedBountyClaim() {
        if (!delayedBountyClaims.isEmpty() && !ProxyDatabase.isWaitingForConnectionData()) {
            ...
            for (int i = 0; i < delayedBountyClaimsCopy.size(); i++) {
                ...
                NaturalBounties.getServerImplementation().global().runDelayed(bountyClaimInstance::claimBounty, 1 + i * 10L);
            }
        }
    }
    ...
    public static void claimBounty(@NotNull Player player, Player killer, List<ItemStack> drops, boolean forceEditDrops, double deathTax) {
        NaturalBounties.debugMessage("Received a bounty claim request.", false);
        if (ProxyDatabase.isWaitingForConnectionData()) {
            NaturalBounties.debugMessage("A bounty claim request was received, but the proxy database is still syncing data. Delaying request.", false);
        ...
        if (killer == null)
            return;
        NaturalBounties.debugMessage(killer.getName() + " killed " + player.getName(), false);

        // check if a bounty can be claimed
        if (!BountyClaimRequirements.canClaim(player, killer)) {
            NaturalBounties.debugMessage("An external plugin, world filter, or a shared team is preventing this bounty from being claimed.", false);
            return;
        }
        if (player == killer) {
            NaturalBounties.debugMessage("Player killed themself. D:", false);
            return;
        }
        if (!killer.hasPermission("NaturalBounties.claim")) {
            NaturalBounties.debugMessage("Player doesn't have the NaturalBounties.claim permission.", false);
            return;
        }
        ...
        if (hasBounty(killer.getUniqueId()) && ConfigOptions.isStealBounties()) {
            ...
            if (!stolenBounty.getSetters().isEmpty()) {
                ...
                NaturalBounties.debugMessage("Killer stole a bounty!", false);
                if (NumberFormatting.getManualEconomy() == ManualEconomy.AUTOMATIC) {
                    // give rewards
                    NaturalBounties.debugMessage("Giving stolen bounty.", false);
        ...
        if (bounty == null) {
            NaturalBounties.debugMessage("Player doesn't have a bounty.", false);
            return;
        }

        // check if it is a npc
        if (!ConfigOptions.isNpcClaim() && killer.hasMetadata("NPC")) {
            NaturalBounties.debugMessage("This is an NPC, which bounty claiming is disabled for in the config.", false);
            return;
        }

        // check if killer can claim it
        if (bounty.getTotalDisplayBounty(killer) < 0.01 && bounty.getTotalItemBounty(killer).isEmpty()) {
            NaturalBounties.debugMessage("This bounty is too small, and there are no items attached!", false);
            return;
        }
        NaturalBounties.debugMessage("Bounty to be claimed: " + bounty.getTotalDisplayBounty(killer), false);

        BountyClaimEvent event1 = new BountyClaimEvent(killer, new Bounty(bounty));
        Bukkit.getPluginManager().callEvent(event1);
        if (event1.isCancelled()) {
            NaturalBounties.debugMessage("The bounty event got canceled by an external plugin.", false);
            return;
        }
        ...
        NaturalBounties.debugMessage("Claim messages sent to all players.", false);
                ...
                if (!givenHead.contains(setter.getUuid())) {
                    ...
                    if (p != null) {
                        ...
                        if (!RewardHead.isRewardKiller() || !Objects.requireNonNull(killer).getUniqueId().equals(setter.getUuid())) {
                            ...
                            NaturalBounties.debugMessage("Gave setter " + p.getName() + " a player skull for the bounty.", false);
                        }
                    } else {
                        // Setter is offline.
                        ...
                        NaturalBounties.getServerImplementation().async().runNow(() -> DataManager.syncPlayerData(playerData.getUuid(), null));
                        NaturalBounties.debugMessage("Will give " + playerData.getPlayerName() + " a player skull when they log on next for the bounty.", false);
                    }
                }
            }
        }
        if (RewardHead.isRewardKiller()) {
            ...
            NaturalBounties.debugMessage("Gave killer " + killer.getName() + " a player skull for the bounty.", false);
        }

        // death tax
        if (deathTax > 0 && NumberFormatting.getManualEconomy() != NumberFormatting.ManualEconomy.PARTIAL) {
            NaturalBounties.debugMessage("Removing " + bounty.getTotalDisplayBounty(killer) * deathTax + " currency for death tax", false);
            ...
            try {
                ...
                NaturalBounties.debugMessage("Player does not have enough currency for the death tax", false);
                try {
                    ...
                    NaturalBounties.debugMessage("Could not remove player's balance.", false);
                }
            }
            if (!removedItems.isEmpty()) {
                ...
                if (totalLoss > 0) {
                    NaturalBounties.debugMessage("Removing " + totalLoss + " currency for the death tax.", false);
        ...
        NaturalBounties.debugMessage("Redeeming Reward: of " + rewardedBounty.getTotalDisplayBounty(), false);
        NaturalBounties.debugMessage(rewardedBounty.toString(), false);
        if (!ConfigOptions.getMoney().getRedeemRewardLater().isVouchers()) {
            ...
            if (ConfigOptions.getMoney().getRedeemRewardLater().isVoucherPerSetter()) {
                NaturalBounties.debugMessage("Handing out vouchers.", false);
                // multiple vouchers
                ...
                NaturalBounties.debugMessage("Handing out a voucher.", false);
                // one voucher
                ItemStack item = new ItemStack(Material.PAPER);
                ItemMeta meta = item.getItemMeta();
                assert meta != null;
        ...
        NaturalBounties.debugMessage("Given stats.", false);
    ...
    private static void giveDelayedReward(Player player, Bounty bounty, int delayMS) {
        NaturalBounties.debugMessage("Delaying the reward for " + player.getName() + " by "
                + LocalTime.formatTime(delayMS, LocalTime.TimeFormat.RELATIVE), false);

        DelayedReward task = new DelayedReward(bounty, player);
        task.setTaskImplementation(NaturalBounties.getServerImplementation().async().runDelayed(task, delayMS / 50));
    }
    ...
    public static void rewardBounty(UUID uuid, Bounty bounty) {
        if (NumberFormatting.getManualEconomy() == NumberFormatting.ManualEconomy.PARTIAL) {
            ...
            NaturalBounties.debugMessage("(Partial Economy) Directly giving auto-bounty reward.", false);
            refundPlayer(uuid, bounty.getBounty(DataManager.GLOBAL_SERVER_ID).getTotalBounty(uuid), Collections.emptyList(), null);
        } else {
            NaturalBounties.debugMessage("Directly giving total claimed bounty.", false);
