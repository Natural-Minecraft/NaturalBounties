...
import me.naturalsmp.NaturalBounties.features.settings.integrations.external_api.LiteBansClass;
...
public class BanChecker {
    ...
    private static void setBanCheckTask() {
        banCheckTask = NaturalBounties.getServerImplementation().global().runAtFixedRate(new Runnable() {
            List<Bounty> bountyListCopy = new ArrayList<>();
            int playersPerRun = 1;
            @Override
            public void run() {
                if (!enabled || NaturalBounties.isPaused())
                    return;
                ...
                NaturalBounties.debugMessage("Checking " + playersPerRun + " players for bans.", false);
                for (int i = 0; i < playersPerRun; i++) {
                    ...
                    NaturalBounties.getServerImplementation().async().runNow(() -> {
                        if (isPlayerBanned(player)) {
                            NaturalBounties.getServerImplementation().global().run(() -> BountyManager.removeBounty(bounty.getUUID()));
                        }
                    });
                }
                if (playersPerRun > 0) {
